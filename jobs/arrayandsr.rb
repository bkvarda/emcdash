...
SCHEDULER.every '1m' do
  ...
  array_counts.each do |array|
   array_hash[array[0]] = { label: array[0], value: array[1].to_s }
  end
  ...
  send_event('total_srs', value: sr_data["rows"].length())
  send_event('array_counts',   { items: array_hash.values })
  send_event('sev_1s', value: sr_counts['S1'] || 0)
  send_event('picture', image: cust_logo)
  send_event('sev1_data', { items: sev1_hash.values })
